...
class QFunction:
    ...
    def _build_graph(self):
        ...
        tf.add_to_collection('pred_Qs', self.output, scope=self.scope)
...
class DDPGFunction:
    def __init__(self, o_space, gamma=0.99, scope='Actor', mode='Q'):
        self._scope = scope
        self._o_space = o_space
    ...
    def _build_graph_q(self):
        self.state = tf.placeholder(dtype=tf.float32, shape=(None, self._action_n), name="state")
        self.action = tf.placeholder(dtype=tf.float32, shape=(None, self._o_space.shape[0]), name="state")
    ...
    def Q(self, state, action): #output Q for the loss
        _state = np.asarray(state).reshape(-1, self._o_space.shape[0])
